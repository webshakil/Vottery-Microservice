...
export class OTP {
  ...
  // Verify OTP ONLY with otp code (no email/identifier)
  static async verify(otpCode, otpType) {
    const client = await pool.connect();
    try {
      // Get OTP record
      const query = `
        SELECT * FROM vottery_otps 
        WHERE otp_code = $1 AND otp_type = $2 AND used_at IS NULL
        ORDER BY created_at DESC
        LIMIT 1
      `;
      
      const result = await client.query(query, [otpCode, otpType]);
      ...
      logger.info(`OTP verified successfully (${otpType})`);
      return { success: true, message: 'OTP verified successfully' };
